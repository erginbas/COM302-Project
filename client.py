...
"""
Black-box channel simulator. (client_lib)

Instructions
------------
python3 client_lib.py --input_file=[FILENAME] --output_file=[FILENAME] --srv_hostname=[HOSTNAME] --srv_port=[PORT]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="COM-302 black-box channel simulator. (client_lib)",
                                     epilog="To promote efficient communication schemes,"
                                            "transmissions are limited to 1 Mega-sample.")
...
if __name__ == '__main__':
    ...
    with socket.socket(family=socket.AF_INET, type=socket.SOCK_STREAM) as sock_cl:
        ...
        elif rx_header[:1] == b'2':  # Outdated version of client_lib.py
            raise Exception(rx_data.tobytes())
        else:  # Unknown header
